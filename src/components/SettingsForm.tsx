...
export function SettingsForm({
  ...
  async function submit(e: React.FormEvent) {
    ...
    const res = await fetch("/api/account", {
      ...
      body: JSON.stringify({ name: form.name, city: form.city, phone: form.phone }),
    });
    setLoading(false);
    if (!res.ok) {
      const data = await res.json().catch(() => ({}));
  ...
  return (
    <form onSubmit={submit} className="card p-5 space-y-4 max-w-xl">
      ...
      <div>
        ...
        <input className="input bg-neutral-50" dir="ltr" value={form.email} disabled />
        <p className="text-xs text-neutral-400 mt-1">البريد هو معرف حسابك ولا يمكن تغييره حالياً</p>
      </div>
