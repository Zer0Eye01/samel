...
export function UserMenu({
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        <Avatar name={name} color={color} src={avatarUrl} className="size-8 text-sm" />
        <ChevronDown
        ...
        <>
          ...
          >
            ...
            <div className="flex items-center gap-3 rounded-2xl bg-neutral-50 max-sm:bg-white/60 p-3 mb-1.5">
              <Avatar name={name} color={color} src={avatarUrl} className="size-11 text-base" />
              <div className="min-w-0">
                <p className="font-bold text-sm truncate">{name}</p>
                <div className="flex items-center gap-1.5 mt-1">
                  {isPro && (
                    <span className="badge bg-neutral-900 text-primary-400 text-[10px]">PRO</span>
                  )}
