...
import { Bell, Gavel, Plus } from "lucide-react";
...
export async function Header() {
  ...
  return (
    ...
    <header className="sticky top-0 z-40 bg-white border-b border-neutral-200/80">
      {/* row 1 — logo · search · actions (generous top padding, bg flush to top) */}
      <div className="container-page flex items-center gap-3 sm:gap-6 pt-5 pb-3 sm:pt-7 sm:pb-4">
        <Link href="/" className="flex items-center gap-2 shrink-0">
          <span className="size-9 sm:size-10 rounded-xl bg-neutral-900 text-white flex items-center justify-center">
            <Gavel className="size-4.5 sm:size-5" />
          </span>
        ...
        <Suspense>
          <SearchBar className="flex-1 max-w-2xl hidden md:block" placeholder={t.searchPh} />
        </Suspense>
