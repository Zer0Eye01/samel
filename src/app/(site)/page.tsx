...
export default async function HomePage() {
  ...
  const now = new Date();
  const [banners, midBanners, categories, liveAuctions, featured, latest, stats] =
    await Promise.all([
  ...
  // listing counts per main category (own + children)
          ...
          <div className="grid grid-cols-4 sm:grid-cols-6 lg:grid-cols-11 gap-2 sm:gap-3">
            {categories.map((cat) => (
              <Link
                ...
                className="flex flex-col items-center gap-2 py-3 px-1 text-center rounded-xl hover:bg-white hover:shadow-card transition-all group"
              >
                <span className="size-11 sm:size-12 rounded-full bg-white border border-neutral-200 text-neutral-600 flex items-center justify-center group-hover:border-primary-500 group-hover:text-primary-600 transition-colors">
                  <CategoryIcon name={cat.icon} className="size-5" />
                </span>
                <span className="font-medium text-[11px] sm:text-xs text-neutral-700 leading-tight">
                  {lang === "en" ? cat.nameEn : cat.nameAr}
                </span>
                <span className="text-[10px] text-neutral-400 -mt-1 max-sm:hidden">
                  {catCount(cat)}
                </span>
              </Link>
            ))}
