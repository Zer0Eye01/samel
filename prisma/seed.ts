...
async function main() {
  ...
  for (const [slug, title, desc, price, cond, city, hood, imgs, featured, views, agoH, seller] of listings) {
    const l = await db.listing.create({
      data: {
        ...
        phone: seller.phone,
        searchText: buildSearchText(title, desc, city),
      },
    });
    created.push(l.id);
  }
    ...
    data: [
      {
        ...
        features: JSON.stringify(["10 إعلانات نشطة", "3 مزادات نشطة", "ظهور عادي في البحث", "إحصائيات أساسية"]),
        maxListings: 10, maxAuctions: 3, sortOrder: 0,
      },
      {
        key: "PRO_MONTHLY", name: "برو شهري", price: 99, period: "شهرياً",
        features: JSON.stringify(["إعلانات غير محدودة", "10 مزادات نشطة", "أولوية في نتائج البحث", "شارة PRO على ملفك", "إحصائيات تفصيلية", "رصيد إعلانات مميزة شهري", "قناة دعم مخصصة"]),
        maxListings: 100000, maxAuctions: 10, highlight: true, sortOrder: 1,
      },
      {
        key: "PRO_YEARLY", name: "برو سنوي", price: 899, period: "سنوياً — وفّر 25%",
        features: JSON.stringify(["كل مزايا برو الشهري", "شهران مجاناً", "أولوية قصوى في الظهور"]),
        maxListings: 100000, maxAuctions: 10, sortOrder: 2,
      },
    ],
  });
  ...
  await db.store.create({
    data: {
      ...
      bannerUrl: "/images/banners/pro.svg",
    },
  });
  ...
  await db.banner.createMany({
    data: [
      { title: "مزادات صامل المباشرة", imageUrl: "/images/banners/auctions.svg", linkUrl: "/auctions", position: "HOME_TOP", status: "ACTIVE" },
      { title: "موسم السيارات في صامل", imageUrl: "/images/banners/cars.svg", linkUrl: "/category/cars", position: "HOME_TOP", status: "ACTIVE" },
      { title: "حساب برو للتجار", imageUrl: "/images/banners/pro.svg", linkUrl: "/pro", position: "HOME_MIDDLE", status: "ACTIVE" },
    ],
  });
  ...
  await db.notification.createMany({
    data: [
      ...
      { userId: demo.id, type: "SYSTEM", title: "أهلاً بك في صامل", body: "اكتمل تفعيل حسابك. ابدأ بتصفح المزادات الحية أو أضف إعلانك الأول.", createdAt: hoursAgo(24 * 300), readAt: hoursAgo(24 * 299) },
    ],
  });
